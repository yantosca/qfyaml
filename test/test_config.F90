...
999 CONTINUE
  ...
  SUBROUTINE Parse_Transport( yml, RC )
    ...
    REAL(yp)                     :: v_real
    INTEGER                      :: match_ct
    CHARACTER(LEN=QFYAML_NamLen) :: a_str(300)
    CHARACTER(LEN=QFYAML_NamLen) :: key
    CHARACTER(LEN=QFYAML_NamLen) :: match_vars(QFYAML_MaxStack)
    ...
    PRINT*, TRIM( key )
    C = FindValidEntries( a_str )
    PRINT*, 'Number of transported species: '
    PRINT*, '==> ', C
    PRINT*, '==> ', a_str(1:C)(1:14)

    !### Also check if there are passive species
    key= "operations%transport%passive_species%"
    CALL QFYAML_FindNextHigher( yml, TRIM( key ), match_ct, match_vars )
    ...
    IF ( match_ct > 0 ) THEN
       DO N = 1, match_ct
          ...
          ! Mol_wt_in_g
          key = TRIM( match_vars(N) ) // '%mol_wt_in_g'
          CALL QFYAML_Add_Get( yml, key, v_str, "", RC )
          PRINT*, TRIM( key )
          READ( v_str, * ) v_real
          v_real = RoundOff( v_real, 2 )
          print*, '===> ', v_real

          ! lifetime_in_s
          key = TRIM( match_vars(N) ) // '%lifetime_in_s'
          CALL QFYAML_Add_Get( yml, key, v_str, "", RC )
          PRINT*, TRIM( key )
          READ( v_str, * ) v_real
          print*, '===> ', v_real
          ...
          ! lifetime_in_s
          key = TRIM( match_vars(N) ) // '%default_bkg_conc_in_vv'
          CALL QFYAML_Add_Get( yml, key, v_str, "", RC )
          PRINT*, TRIM( key )
          READ( v_str, * ) v_real
          print*, '===> ', v_real
  ...
  SUBROUTINE Parse_Aerosols( yml, RC )
    ...
    DO N = 1, SIZE( tags )
       ...
       ELSE IF ( INDEX( key, TRIM( tags(8) ) ) > 0 ) THEN
          ...
          IF ( RC /= QFYAML_Success ) GOTO 999
          READ( a_str(1), * ) a_real(1)
          READ( a_str(2), * ) a_real(2)
...
999 CONTINUE
    RC = QFYAML_Failure
    print*, "Error in Parse_Strat_Aerosols!"
    RETURN
  ...
  SUBROUTINE Parse_ObsPack( yml, RC )
    ...
    DO N = 1, SIZE( tags )
       ...
       ELSE IF ( INDEX( key, TRIM( tags(5) ) ) > 0 ) THEN
          ...
          C = 0
          DO V = 1, SIZE( a_str )
             IF ( TRIM( a_str(V) ) == MISSING_STR ) EXIT
             C = C + 1
          ENDDO
...
999 CONTINUE
  ...
  FUNCTION FindValidEntries( a_str ) RESULT( ct )
    !
    ! Finds the number of entries in a string array
    ! that do not match the MISSING_INT value
    !
